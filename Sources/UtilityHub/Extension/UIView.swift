...
public extension UIView {
    ...
    func setShadow(color: UIColor? = .secondaryLabel, background: UIColor? = .systemBackground, offset: CGSize = .zero, opacity: Float = 1, radius: CGFloat = 10, corners: CACornerMask = [.layerMinXMinYCorner, .layerMaxXMinYCorner, .layerMaxXMaxYCorner, .layerMinXMaxYCorner], cornerRadius: CGFloat = 0, path: CGPath? = nil) {
        roundedCorner(corners: corners, radius: cornerRadius, masks: true, clips: false)
...
public extension UIView {
    enum DashOrientation {
        case horizontal
        case vertical
    }
    
    func makeDashedBorderLine(color: UIColor = .color163BDE, fillColor: UIColor = .clear, strokeLength: NSNumber = 2, gapLength: NSNumber = 4, width: CGFloat = 1, orientation: DashOrientation = .vertical) {
        let path = CGMutablePath()
        
        let shapeLayer = CAShapeLayer()
        
        shapeLayer.name = "DashBorderLine"
    ...
    func addDashBorder(lineJoin: CAShapeLayerLineJoin = .round, dashColor: UIColor = .color163BDE, fillColor: UIColor = .clear, strokeLength: NSNumber = 2, gapLength: NSNumber = 4, width: CGFloat = 1, cornerRadius: CGFloat = 0) {
        let shapeRect = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
        
        let path = UIBezierPath(roundedRect: shapeRect, cornerRadius: cornerRadius).cgPath
...
public extension UIView{
    ...
    /**
     ...
     - Parameters:
        ...
        - width: Receives `Optional<CGFloat>` or `CGFloat?`, Default value is `nil`
        ...
        - height: Receives `Optional<CGFloat>` or `CGFloat?`, Default value is `nil`
        - maxHeight: Receives `Optional<CGFloat>` or `CGFloat?`, Default value is `nil`
     */
    func setDimensions(minWidth: CGFloat? = nil, width: CGFloat? = nil, maxWidth: CGFloat? = nil, minHeight: CGFloat? = nil, height: CGFloat? = nil, maxHeight: CGFloat? = nil) {
        translatesAutoresizingMaskIntoConstraints = false
    ...
    func setPadding(top: CGFloat, left: CGFloat, bottom: CGFloat, right: CGFloat) {
        // Create a new wrapper view
        let wrapperView = UIView()
        wrapperView.translatesAutoresizingMaskIntoConstraints = false
        
        // Move existing subviews to the wrapper view
        ...
        // Apply padding to the wrapper view
        NSLayoutConstraint.activate([
            wrapperView.topAnchor.constraint(equalTo: self.topAnchor, constant: top),
            wrapperView.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: left),
            wrapperView.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -bottom),
            wrapperView.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -right)
        ])
    }
}

public extension UIView {
    func applyBlurEffect(_ style: UIBlurEffect.Style = .systemUltraThinMaterialDark) {
